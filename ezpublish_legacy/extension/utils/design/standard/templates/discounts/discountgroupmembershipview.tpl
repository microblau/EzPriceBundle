<form action={'basket/discountgroup'|ezurl} method="post">
...
<tr>
    ...
    <th>{'Inicio'|i18n( 'design/admin/shop/discountgroupmembershipview' )}</th>
    <th>{'Fin'|i18n( 'design/admin/shop/discountgroupmembershipview' )}</th>
    <th class="tight">&nbsp;</th>
</tr>
...
<tr class="{$Rules.sequence}">
    ...
    <td>{$Rules.item.start|datetime( 'custom', '%d/%m/%Y')}</td>
    <td>{$Rules.item.end|datetime( 'custom', '%d/%m/%Y')}</td>
