                        ...
                        if( isset( $params[1] ) )
                            ...
                            {                            	
                                ...
                                foreach( $items as $item )
                                {
                                	...
                                	if( $http->hasPostVariable( 'Pos_' . $block->attribute( 'id' ) . '_' . $objectID ) )
                                	{                  
                                                
                                        $item->setAttribute( 'pos', $http->postVariable( 'Pos_' . $block->attribute( 'id' ) . '_' . $objectID ) );
                                        $db = eZDB::instance();
                                        $db->query( 'UPDATE ezm_pool_efl SET pos = ' . $http->postVariable( 'Pos_' . $block->attribute( 'id' ) . '_' . $objectID ) . ' 
                                        		    WHERE block_id = "' . $block->attribute( 'id' ) .  '"
                                ...
                                foreach ( $block->attribute( 'items' ) as $item )
                                {
                                    ...
                                    switch ( $action )
                                    {
                                        ...
                                        case 'add':
                                            ...
                                            {
                                                $check = customEZFlowFunctionCollection::getPosForNodeInBlock ( $blockID, $item->attribute( 'node_id' ) ) ;
                                            ...
                                            VALUES ( '" . $blockID . "',
                                                ...
                                                if( $block->attribute( 'type' ) == 'Completo2Modalidades' )
                                                {
                                                   
                                                   
                                                   
                                                	$db->query( "INSERT INTO ezm_pool_efl 
                                                	VALUES ( '" . $blockID . "', 
                                                		     '" . $item->attribute( 'object_id' )  . "',
                                                    		 '" . $item->attribute( 'node_id' ) . "',                                                     		 
                                                     		 '" . $check['result']['pos'] . "'  )
        ...
        if ( $http->hasPostVariable( $base . '_ezpage_block_name_array_' . $contentObjectAttribute->attribute( 'id' ) ) )
        {
            ...
            foreach ( $blockNameArray as $zoneID => $blocks )
            {
                ...
                foreach ( $blocks as $blockID => $blockitem )
                {
                    ...
                    $items = $blockitem->attribute('waiting' );
                    foreach( $items as $item )
                    {
                    
                        if ( $http->hasPostVariable( 'Pos_' . $block->attribute( 'id' ) . '_' . $item->attribute( 'object_id' ) ) )
                        {
                            ...
                            $db = eZDB::instance();
                                         $db->query( "INSERT INTO ezm_pool_efl 
                                                    	VALUES ( '" . $blockID . "', 
                                                    		     '" . $item->attribute( 'object_id' )  . "',
                                                        		 '" . $item->attribute( 'node_id' ) . "',                                                     		 
                                                         		 '" . $http->postVariable( 'Pos_' . $block->attribute( 'id' ) . '_' .  $item->attribute( 'object_id' ) ) . "'  )
                                                         ON DUPLICATE KEY UPDATE 
                                                         block_id = '" . $blockID . "', 
                                                         pos = " . $http->postVariable( 'Pos_' . $block->attribute( 'id' ) . '_' .  $item->attribute( 'object_id' ) )
                                                         		 );
                        }
                    }
