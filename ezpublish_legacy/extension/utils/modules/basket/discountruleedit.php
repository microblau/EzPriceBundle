...
if ( $http->hasPostVariable( 'BrowseProductButton' ) )
{
    eZContentBrowse::browse( array( 'action_name' => 'FindProduct',
                                    ...
                                    'from_page' => "/basket/discountruleedit/$discountGroupID/$discountRuleID" ),
                             $module );
    return;
}
...
else
{
    ...
    if ( $discountRuleID )
    {
        ...
        $discountRule = eZCustomDiscountSubRule::fetch( $discountRuleID );
        if ( !$discountRule )
        {
            return $Module->handleError( eZError::KERNEL_NOT_AVAILABLE, 'kernel' );
        }
...
if ( $http->hasPostVariable( 'StoreButton' ) )
{
    ...
    $db = eZDB::instance();
    $db->begin();
    $aux = explode('/', $http->postVariable( 'start' ) );
    $start = mktime( 0, 0, 0, $aux[1], $aux[0], $aux[2] );
    $aux1 = explode('/', $http->postVariable( 'end' ) );
    $end =  mktime( 23, 59, 59, $aux1[1], $aux1[0], $aux1[2] );
    ...
    {
        $discountRule = eZCustomDiscountSubRule::create( $discountGroupID );
        $validRules = eZPersistentObject::fetchObjectList( eZDiscountSubRule::definition(), array(), null,
        ...
        $discountRule->setAttribute( 'id', $validRules[0]['max'] + 1 );
        $discountRule->store();
    ...
    $aux = explode('/', $http->postVariable( 'start' ) );
    $start = mktime( 0, 0, 0, $aux[1], $aux[0], $aux[2] );
    $discountRule->setAttribute( 'start', $start );
    $aux1 = explode('/', $http->postVariable( 'end' ) );
    $end =  mktime( 23, 59, 59, $aux1[1], $aux1[0], $aux1[2] );
    $discountRule->setAttribute( 'end', $end );
    
    if( ( $start < time() ) and ( $end > time() ) )
    {
        $valid = new eZDiscountSubRule( array( 
                                'id' => $discountRuleID,
                                'discountrule_id' => $discountGroupID,
                                'name' => trim( $http->postVariable( 'discountrule_name' ) ),
                                'discount_percent' =>  $http->postVariable( 'discountrule_percent' ),
                                'limitation' => '*'
        ) );
    ...
    if ( $http->hasPostVariable( 'Products' ) && $http->postVariable( 'Products' ) )
    {
        ...
        if( ( $start < time() ) and ( $end > time() ) )
        {   
            $valid->setAttribute( 'limitation', false );
        }
    }
    else
    {
    ...
    {        
        if( $rule = eZDiscountSubRule::fetch( $discountRule->attribute( 'id' ) ) )
            $rule->remove( $rule->ID );
    }
...
{
    $aux = eZCustomDiscountSubRule::fetch( $discountRule['id'] );
}
else
{
    $aux = eZCustomDiscountSubRule::fetch( $discountRule->attribute( 'id' ) );
}



$tpl->setVariable( 'start', ( $aux->Start > 0 ) ? date( 'd/m/Y', $aux->Start ) : '' );
$tpl->setVariable( 'end', ( $aux->End > 0 ) ? date( 'd/m/Y', $aux->End ) : '' );

$Result = array();
$Result['content'] = $tpl->fetch( 'design:discounts/discountruleedit.tpl' );
