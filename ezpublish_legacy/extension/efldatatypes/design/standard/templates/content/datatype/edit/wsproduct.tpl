{ezscript_require('wsproduct_jquery.js')}
...
html_class='full'}
    ...
    <input id="ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}_id" type="hidden" name="{$attribute_base}_ezstring_data_int_{$attribute.id}" value="{$attribute.data_int|wash( xhtml )}" />
{/default}
