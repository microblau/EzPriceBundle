...
<div id="commonGrid" class="clearFix">
                ...
                <div id="subNavBar">
                    ...
                    <ul>
                        ...
                        {include uri='design:formacion/menu.tpl' check=$node actual=fetch( 'content', 'list', hash( 'parent_node_id', $node.node_id,
                                                                                                                            'attribute_filter', array( array( 'name', '=', 'Por área' ) )
                        )).0 hijo=$node.name|normalize_path()|explode('_')|implode('-')}              
                    </ul>
 ...
 {def $number_of_items=cond( ezpreference( 'products_per_page')|ne(''),   ezpreference( 'products_per_page'), 5 )                                       
                              ...
                              {def $results = fetch( 'ezfind', 'search', hash( 'query', '',
                                                                                         'subtree_array', array( 62 ),
                                                                                         'class_id', array( 49, 66, 61, 94 ),
                                                                                         'limit', $number_of_items,
                                                                                         'sort_by', $sort_array,
                                                                                         'filter', 'attr_curso_medida_b:true',
                                ...
                                <div class="description busquedaCursos">
                                    <ul class="wrapAjaxContent clearFix">
                                         ...
                                         {foreach $elements as $index => $element}
                                            ...
                                            {node_view_gui content_node=$element view=line reset=$index|eq( $elements|count|sub(1) ) nodefrom=$node.node_id}
                                         {/foreach}
