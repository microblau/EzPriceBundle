...
<div class="block-parameters float-break">
    ...
    {elseif $is_custom}
        {def $custom_attributes = ezini( $block.type, 'CustomAttributes', 'block.ini' )
        	 $custom_titles = ezini( $block.type, 'CustomAttributes_title', 'block.ini' )
        ...
        {foreach $custom_attributes as $custom_attrib}
            ...
            {else}
                ...
                <label>{$custom_titles[$custom_attrib]}:</label> 
...
<table border="0" cellspacing="1" class="items queue" id="z:{$zone_id}_b:{$block_id}_q">
    ...
    <tr id="z:{$zone_id}_b:{$block_id}_i:{$item.object_id}" class="{if $item.ts_publication|lt($current_time)}tbp{/if}">
        ...
        <td>{def $el = fetch( 'portadas', 'get_pos_for_node_in_block', hash( 'block_id', $block.id, 'node_id', $item.node_id ))}
           
        	<select name="Pos_{$block.id}_{$item.object_id}">
        		<option value="1" {if $el.pos|eq(1)}selected="selected"{/if}>1</option>
        		<option value="2" {if $el.pos|eq(2)}selected="selected"{/if}>2</option>
        	</select>
        	{undef $el}
        </td>
            <td class="time">
...
<table border="0" cellspacing="1" class="items online" id="z:{$zone_id}_b:{$block_id}_o">
    <tbody>
    {def $valid_nodes = fetch( 'portadas', 'custom_valid_nodes', hash( 'block_id', $block.id ) )}
    
    {if $valid_nodes|count()}
    {foreach $valid_nodes as $item sequence array( 'bglight', 'bgdark') as $style}
    ...
    {else}
    <tr class="empty">
        <td colspan="4">{'Online: no items.'|i18n( 'design/standard/block/edit' )}</td>
    </tr>
    {/if}
    <tr class="rotation">
        <td colspan="4">{'Rotation:'|i18n( 'design/standard/block/edit' )} <input class="textfield block-control" type="text" name="RotationValue_{$block_id}" value="{$block.rotation.value}" size="5" />
...
<div class="block-controls float-break">
    <div class="left">
     ...
     <div class="left">
        <input class="button block-control" type="submit" name="CustomActionButton[{$attribute.id}_modify_item-{$zone_id}-{$block_id}]" value="Modificar posiciones" />
    </div>
