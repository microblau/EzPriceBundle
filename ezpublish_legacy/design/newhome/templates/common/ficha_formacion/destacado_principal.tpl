            ...
            <div class="inner">
                <div class="wysiwyg">
		            ...
		            {if $node.data_map.curso_subvencionable.content|eq(1)}
                             <div class="pexperto">Curso subvencionable</div>
                            {/if}
                        ...
                        <div class="column3">                                    
                            <div class="precios">
                              ...
                              ddd 
{$node.data_map.precio.content|attribute(view)}                                    <div class="descuentoType1"><span>{$node.data_map.precio.content.discount_price_ex_vat|l10n(clean_currency)} &euro;</span> + IVA</div>
                                {/if}
                                {if $node.data_map.precio.has_content}
                                    <div class="antes">{if $node.data_map.precio.content.has_discount}Antes {/if}<span>{$node.data_map.precio.content.price|l10n(clean_currency)} &euro;</span> + IVA</div>
                                {/if}
                            </div>
