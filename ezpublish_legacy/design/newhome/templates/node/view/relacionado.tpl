...
{if $node.object.contentclass_id|eq(99)}
    ...
    <div class="description">
        ...
        <span class="precio">{if $node.parent.data_map.precio.content.has_discount}<s>{/if}{$node.parent.data_map.precio.content.price|l10n(clean_currency)} €{if $node.parent.data_map.precio.content.has_discount}</s>{/if}
        {if $node.parent.data_map.precio.content.has_discount}<span class="preciooferta">{$node.parent.data_map.precio.content.discount_price_ex_vat|l10n('clean_currency')} €</span>{/if}
        </span>
    </div>
{else}
