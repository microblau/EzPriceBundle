...
$path = explode( '/(', str_replace( 'http://' . $_SERVER['HTTP_HOST'] . '/', '', $http->sessionVariable( "FromPage" )  ) );
$node = eZContentObjectTreeNode::fetchByURLPath( str_replace( '.', '', str_replace( '-', '_', $path[0] ) ) );


if ( ( $ini->variable( 'ShopSettings', 'RedirectAfterAddToBasket' ) == 'reload' ) and ( $node->ClassIdentifier != 'producto' ) and ( $node->ClassIdentifier != 'producto_nautis' )  and ( $node->ClassIdentifier != 'version_nautis' )  and ( $node->ClassIdentifier != 'producto_nautis' )  )
