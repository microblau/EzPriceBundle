...
{def $node_url_alias      = $node.url_alias
     ...
     $valid_tabs = array( $default_tab, 'details', 'translations', 'locations', 'relations', 'states', 'fezmetadata' )
}
...
<ul class="tabs">
    ...
    <li id="node-tab-relations" class="middle{if $node_tab_index|eq('relations')}  selected{/if}">
     ...
     {* Metadata *}
    <li id="node-tab-fezmetadata" class="middle{if $node_tab_index|eq('fezmetadata')}  selected{/if}">
        <a href={concat( $node_url_alias, '/(tab)/fezmetadata' )|ezurl} title="{'Show metadatas.'|i18n( 'design/admin/node/view/full' )}">MetaData</a>
    </li>
