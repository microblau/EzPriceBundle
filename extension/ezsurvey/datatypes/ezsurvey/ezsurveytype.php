    ...
    function processViewActions( $objectAttribute, &$survey, &$validation )
    {
        ...
        if ( $continueViewActions === true )
        {
            ...
            if ( $actionContinue === true )
            {
                ...
                $http->hasPostVariable( $postSurveyStoreButton );
              
                if ( ( $http->hasPostVariable( $postSurveyStoreButton ) || $http->hasPostVariable( $postSurveyStoreButton . '_x' ) ) && $validation['error'] == false )
                {
                    $result->storeResult( $params );
                    $dbaux = eZDB::instance();
                   $n = $http->postVariable( 'pid' );
...
$info = unserialize( $orderInfo->Order );
                    ...
                    $dbaux->query( "INSERT INTO ezsurveyresult_efl VALUES(" . $result->attribute( 'id' ) . ", " . $info['id_pedido_lfbv'] . ", " . $http->sessionVariable( 'id_user_lfbv' ) . ") ");
