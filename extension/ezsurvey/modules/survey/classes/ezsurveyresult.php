...
class eZSurveyResult extends eZPersistentObject
{
    ...
    */
    ...
    static function exportCSV( $contentObjectID, $contentClassAttributeID, $languageCode )
    {
        ...
        $indexList[] = 'id_pedido_lfbv';
        $questions['id_pedido_lfbv'] = 'Id pedido Lefebvre';        
        $indexList[] = 'id_user_lfbv';
        $questions['id_user_lfbv'] = 'Id usuario Lefebvre';
        ...
        foreach( array_keys( $rows ) as $key )
        {
            ...
            $data = $dbaux->arrayQuery( "SELECT survey_id, id_pedido_lfbv,	id_user_lfbv FROM ezsurveyresult_efl WHERE survey_id = " . $row['result_id' ] );
            if( $data[0]['survey_id'] )
            {
                $answers['id_user_lfbv'] = $data[0]['id_user_lfbv'];
                $answers['id_pedido_lfbv'] = $data[0]['id_pedido_lfbv'];
            }
        }
        if ( $oldID !== false )
        {
