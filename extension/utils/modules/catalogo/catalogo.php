...
$Offset = isset( $Params['Offset'] ) ? $Params['Offset'] : $Params['UserParameters']['offset'] ;
$Year = $Params['Year'];
$Month = $Params['Month'];
$Day = $Params['Day'];
...
if ( isset( $paramtonodes[$param1] ) and ( $node = eZContentObjectTreeNode::fetch( $paramtonodes[$param1] ) ) )
{	
	...
	$tpl->setVariable( 'attribute', $attributes[$param1] );
	
	// seteamos el texto a mostrar en resultados
	$tpl->setVariable( 'text', $texts[$param1] );
...
{
    $solr = new eZSolr();
    $result = $solr->search( '', array( 'SearchSubTreeArray' => array( 61 ),
