...
if ( $http->hasPostVariable( "RemoveRuleButton" ) )
{
    ...
    foreach ( $discountRuleIDList  as $discountRuleID )
    {
        ...
        eZCustomDiscountSubRule::remove( $discountRuleID );
    }
    $db->commit();
...
if ( $http->hasPostVariable( "AddCustomerButton" ) )
{
    eZContentBrowse::browse( array( 'action_name' => 'AddCustomer',
                                    ...
                                    'from_page' => "/basket/discountgroupview/$discountGroupID" ),
                             $module );
    return;
}
...
$ruleList = eZCustomDiscountSubRule::fetchByRuleID( $discountGroupID );
...
$Result = array();
$Result['content'] = $tpl->fetch( "design:discounts/discountgroupmembershipview.tpl" );
