...
do
{
    ...
    for( $i = 0; $i < count( $part ) ; $i++ )
    {
        ...
        $vector[] = 'http://' . $_SERVER['HTTP_HOST'] . '/' . $nodo->urlAlias() . '?utm_source=kelkoo&utm_medium=cpc&utm_content=Kelkoo&utm_campaign=Kelkoo' . $data['referencia']->content();
...
header('Content-Disposition: attachment; filename="fichero_kelkoo_EFL.txt"');
echo $output;
echo "\r\n";
