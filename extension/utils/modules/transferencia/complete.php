...
if( $http->hasPostVariable( 'btnContinuar' ) )
{
    ...
    $Result = array();
    $http->removeSessionVariable( 'id_pedido_lfbv' );
...
elseif ( $product['item_object']->attribute( 'contentobject' )->attribute( 'contentclass_id' ) == eZINI::instance( 'qmementix.ini' )->variable( 'Qmementix', 'Class' ) )
                        ...
                        $products_to_ws[] = array(
                            'orden' => $i,
                            'ref' => isset( $data['referencia'] ) ? $data['referencia']->content() : '-',
    ...
    $text = new ezcMailText( $tplmail->fetch( 'design:basket/mails/transferencia.tpl' ), 'utf-8' );     
    $text->subType = 'html';                
    $mail->body = $text;
