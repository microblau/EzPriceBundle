 ...
 * @version 1.0.0beta2 | $Id: subscribe.php 12408 2010-07-09 06:35:24Z felix $
...
if ( $module->isCurrentAction( 'Subscribe' ) )
{
	...
	if ( $http->hasPostVariable( 'organisation' ) )
        $subscriptionDataArr['organisation'] = trim( $http->postVariable( 'organisation' ) );	
	//about_info tanta  
   if (( $http->hasPostVariable( 'publicaciones' )) || ($http->hasPostVariable( 'formacion' )))
   {
        $valor_publicaciones = trim( $http->postVariable( 'publicaciones' ) );	
		if ( $valor_publicaciones == 'on')
			$valor_publicaciones = 'publicaciones';
		$valor_formacion = trim( $http->postVariable( 'formacion' ) );	
		if ( $valor_formacion == 'on')
			$valor_formacion = 'formacion';
				
		$subscriptionDataArr['about_info'] =$valor_publicaciones.'-'.$valor_formacion;
	} else{
		$subscriptionDataArr['about_info'] ='publicaciones-formacion';
	}
	
	//about_materia tanta  
/*
	$materias = array('Administrativo', 'Contable', 'Fiscal', 'Inmobiliario', 'Jurídico', 'Mercantil', 'Otras', 'RRHH', 'Social');  
    $materias_numbers = array( 'Administrativo' => 
	$aboutmateria = array();
	$cont = 0; 
	foreach ($materias as $materia) {
		if ( $http->hasPostVariable( $materia )){
				$aboutmateria[]=$materia;
				$count++;
		}//if
	}//foreach

	$subscriptionDataArr['about_materia']='';
	foreach ($aboutmateria as $about){
		$subscriptionDataArr['about_materia'] .= $about .'-';	
	}
	if ($count == 0) {
		$subscriptionDataArr['about_materia'] = 'Administrativo-Contable-Fiscal-Inmobiliario-Jurídico-Mercantil-Otras-RRHH-Social' ;
	}*/
    $subscriptionDataArr['about_materia'] = array();
    $subscriptionDataArr['about_materia'] = implode( '-', $http->postVariable( 'areas' ) );
    if( empty( $subscriptionDataArr['about_materia'] ) )
    {
           
        $areas = eZContentObjectTreeNode::subTreeByNodeId( array( 'AsObject' => false ), 143 );
        $areasids = array();
        foreach( $areas as $area )
        {
             $areasids[] = $area['contentobject_id'];
        }    
        $subscriptionDataArr['about_materia'] = implode( '-', $areasids );
    }
...
//fin about_materia		
 ...
 //   $subscriptionDataArr['list_output_format_array'] = array();
	...
	// acepto tanta
	if( $http->hasPostVariable( 'Acepto' ) ){
        ...
        switch ( $fieldName )
        {
			 ...
			 case 'acepto':
            {
             if ( !$subscriptionDataArr['Acepto'] )
                {
                    $warningArr['acepto'] = $messageArray['acepto'];
                }
            } break;
            default:
        }
    ...
    if( $postedEmailIsFromLoggedInUser === true )
    {
        ...
        if ( count( $warningArr ) == 0 )
        {
            ...
            $tpl->setVariable( 'ebook', $http->postVariable( 'ebook' ) );
            $http->setSessionVariable( 'ebook', $http->postVariable( 'ebook' ) );
...
/*            if ( $mailSendResult['send_result'] === true )
        ...
        else if ( count( $warningArr ) == 0 )
        {
            ...
            $newNewsletterUser = CjwNewsletterUser::fetchByEmail( $subscriptionDataArr['email'] );
            $hash = $newNewsletterUser->Hash;
            $book = $http->postVariable( 'ebook' );
            $db = eZDB::instance();
           
            
            $db->query( "INSERT INTO ebooks_regalo VALUES ( '$hash', $book )");
