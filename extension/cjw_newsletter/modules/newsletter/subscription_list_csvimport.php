 ...
 * @version 1.0.0beta2 | $Id: subscription_list_csvimport.php 12412 2010-07-09 07:20:34Z felix $
    ...
    if ( $importCsvFile == true )
    {
        ...
        foreach ( $csvDataArray as $rowId => $item )
        {
            ...
            if( isset( $item[ 'organisation' ] ) )
                $organisation = utf8_encode($item[ 'organisation' ]);
            else
                $salutation = 0;

            if( isset( $item[ 'first_name' ] ) )
                $firstName = utf8_encode($item[ 'first_name' ]);
            else
                $firstName = '';

            if( isset( $item[ 'last_name' ] ) )
                $lastName = utf8_encode($item[ 'last_name' ]);
            else
                $lastName = '';
				
			if( isset( $item[ 'about_info' ] ) )
               $about_info = utf8_encode($item[ 'about_info' ]);
            else
                $about_info = 0;
				
			if( isset( $item[ 'about_materia' ] ) )
               $about_materia = utf8_encode($item[ 'about_materia' ]);
			 else
                $about_materia = 0;		
