<div id="gridType3">
        ...
        {def $versiones = fetch( 'content', 'list', hash( 'parent_node_id', $producto.node_id,
                                            	...
                                            	<span class="precio">Desde {if $versiones.0.data_map.precio.content.has_discount}<s>{/if}{$$versiones.0.data_map.precio.content.price|l10n(clean_currency)} € + IVA{if $versiones.0.data_map.precio.content.has_discount}</s>{/if}
{if $versiones.0.data_map.precio.content.has_discount}<br /><span class="price_offer">{$versiones.0.data_map.precio.content.discount_price_ex_vat|l10n(clean_currency)}  € + IVA{/if}
                                                    {else}
                                                    <span class="precio">{$producto.data_map.precio.content.price|l10n(clean_currency)} €</span>
                                                    {/if}
                                                </div> 
