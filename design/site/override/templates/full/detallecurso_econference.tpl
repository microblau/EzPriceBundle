...
>Volver al listado</a></span>
                            <ul class="tabs">
                                ...
                                {if $node.data_map.contenidos.has_content}
                                <li {if and(is_set( $view_parameters.info), $view_parameters.info|eq('contenidos') )}class="sel"{/if}>
                                    {if and(is_set( $view_parameters.info), $view_parameters.info|eq('contenidos') )}<h2>{else}<a href="{concat($node.url_alias, '/(info)/contenidos')|ezurl(no)}{if is_set($view_parameters.cursos)}/(cursos)/{$view_parameters.cursos}{/if}#cursoDetalle">{/if}Contenidos{if and( is_set($view_parameters.info), $view_parameters.info|eq('contenidos') )}</h2>{else}</a>{/if}
                                </li>
                                {/if}
                                
                                                                
                                {if $node.data_map.consideraciones.has_content}
                                <li {if and(is_set( $view_parameters.info), $view_parameters.info|eq('consideraciones') )}class="sel"{/if}>
                                    {if and(is_set( $view_parameters.info), $view_parameters.info|eq('consideraciones') )}<h2>{else}<a href="{concat($node.url_alias, '/(info)/consideraciones')|ezurl(no)}{if is_set($view_parameters.cursos)}/(cursos)/{$view_parameters.cursos}{/if}#cursoDetalle">{/if}Consideraciones{if and(is_set( $view_parameters.info), $view_parameters.info|eq('consideraciones') )}</h2>{else}</a>{/if}
                                </li>
                                {/if}
                            ...
                            <div class="cont cursoDet clearFix">
                                       ...
                                       <li>
                                          ...
                                          {if $pon.data_map.foto_ponente.has_content}
                                               <div class="frt">
                                                   ...
                                                   {$foto_ponente.data_map.image.content.original|attribute(show,1)}
                                                   <img src={$foto_ponente.data_map.image.content.testimonio.url|ezroot()} alt="{$foto_ponente.data_map.image.content.original.alternative_text}"/>
                                                   {undef $foto_ponente}
                                               </div>
                                          {/if}
                                          *}
                                ...
                                <div class="column1">
                                    ...
                                    {elseif and(is_set($view_parameters.info), $view_parameters.info|eq('contenidos'))}
                                       {*<h2>Contenidos</h2>*}
                                        {$node.data_map.contenidos.content.output.output_text}
                                    {elseif and(is_set($view_parameters.info), $view_parameters.info|eq('consideraciones'))}
                                       {*<h2>Consideraciones</h2>*}
                                       {$node.data_map.consideraciones.content.output.output_text}
                                    {/if}
