...
{ezpagedata_set( 'metadescription', $node.data_map.subtitulo.content )}     
								...
								<div class="content-view-embed">
                                	...
                                	<div class="column2">
                                    	{$node.data_map.entradilla.content.output.output_text}
                                   		{if or(  $node.parent_node_id|eq(66), $node.parent_node_id|eq(1485), $node.parent_node_id|eq(69), $node.parent_node_id|eq(70), $node.parent_node_id|eq(4058) )|not }
 ...
 {def $discount_type = fetch( 'basket', 'get_discount_type', 
										...
										{else}
										    <p>{$discount_type.name}</p>
										{/if}

                                        <div><div class="precioAnterior"><span><s>{$node.data_map.precio.content.price|l10n(clean_currency)} €</s></span></div> <div>{$node.data_map.precio.content.discount_price_ex_vat|l10n(clean_currency)} €</div></div></div>
                                        {else}
...
{/if}
                                        ...
                                        <div id="modInformacion">
                                            ...
                                            <div>
                                                <span class="verMas"><a href={fetch('content', 'node', hash('node_id', 1456)).url_alias|ezurl()}>Nosotros nos ponemos en contacto con usted</a></span>
                                            </div>
                                        </div>
                                        {/if}
...
>Volver al listado</a></span>
                            <ul class="tabs">
                                ...
                                <li {if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('sumario') )}class="sel"{/if}>
                                	{if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('sumario') )}<h2>{else}

<a href="{concat( $node.url_alias, '/(ver)/sumario')|ezurl(no)}#producttext">

{/if}Sumario{if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('sumario') )}</h2>{else}</a>{/if}
                                </li>
                                {/if}
                                ...
                                {if $node.parent_node_id|eq(66)}
                                <li {if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('novedades') )}class="sel"{/if}>
                                	{if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('novedades') )}<h2>{else}<a href="{concat( $node.url_alias, '/(ver)/novedades')|ezurl(no)}#producttext">{/if}Últimas noticias{if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('novedades') )}</h2>{else}</a>{/if}
                                </li>
                                {/if}

                                 
                                {if $node.data_map.actualizaciones.has_content}
                                <li {if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('actualizaciones') )}class="sel"{/if}>
                                	{if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('actualizaciones') )}<h2>{else}<a href="{concat( $node.url_alias, '/(ver)/actualizaciones')|ezurl(no)}#producttext">{/if}Actualizaciones{if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('actualizaciones') )}</h2>{else}</a>{/if}
                                </li>
                                {/if}
                                
                                {if $testimonios|count|gt( 0 )}
                                <li {if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('testimonios') )}class="sel"{/if}>
                                	{if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('testimonios') )}<h2>{else}<a href="{concat( $node.url_alias, '/(ver)/testimonios')|ezurl(no)}#producttext">{/if}Los clientes opinan{if and( is_set( $view_parameters.ver ), $view_parameters.ver|eq('testimonios') )}</h2>{else}</a>{/if}
                                </li>
                                {/if}
...
<li>
                                    	...
                                    	{if $testimonio.data_map.foto_testimonio.has_content}
                                        ...
                                        {/if}
                                        <q>"{$testimonio.data_map.testimonio.content.output.output_text|strip_tags()}"</q>
                                        <span><span class="nombre">{$testimonio.data_map.nombre_persona.content}. </span>{$testimonio.data_map.empresa.content}</span></li>
					{/foreach}
                                    	</ul>
