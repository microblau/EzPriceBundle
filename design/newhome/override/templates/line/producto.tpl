									<li class="clear{if $reset} reset{/if}" itemscope itemtype="http://schema.org/Book">
											{if $node.data_map.imagen.has_content}
                                   			{def $imagen = fetch( 'content', 'object', hash( 'object_id', $node.data_map.imagen.content.relation_browse.0.contentobject_id ))}                                    
                                   
                                        	<div class="image flt">                                            	
                                            	<img itemprop="image thumbnailUrl" src={$imagen.data_map.image.content.listadoproductos.url|ezroot()} width="{$imagen.data_map.image.content.listadoproductos.width}" height="{$imagen.data_map.image.content.listadoproductos.height}" class="producto" />
                                            </div>
                                            {undef $imagen}
                                            {else}
                                            {def $imagen = fetch( 'content', 'object', hash( 'object_id', 2084))}                                    
                                   
                                        	<div class="image flt">                                            	
                                            	<img src={$imagen.data_map.image.content.listadoproductos.url|ezroot()} width="{$imagen.data_map.image.content.listadoproductos.width}" height="{$imagen.data_map.image.content.listadoproductos.height}" class="producto" />
                                            </div>
                                            {/if}
                                            <div class="wysiwyg flt" >
                                            	<h3><a href={$node.url_alias|ezurl()}>{$node.name}</a></h3>

								{def $cuantasvaloracionestotales = fetch('producto','cuantasvaloraciones' , hash( 'node_id', $node.node_id ))} 
                                {if $cuantasvaloracionestotales|gt(0)}
                                              <h2><a href={concat($node.url_alias, '/valoraciones')|ezroot()}>{$cuantasvaloracionestotales} {if $cuantasvaloracionestotales|eq(1)} valoración{else} valoraciones{/if} de usuarios</a></h2> 
								{/if}
                                               
                                               
                                            	<p>{$node.data_map.subtitulo.content}</p>
                                                <div itemprop="about">
                                            	{$node.data_map.entradilla.content.output.output_text} 
                                                </div>
                                            </div>
                                            <div class="action flt" {if $node.data_map.precio.content.has_discount}itemprop="offers" itemscope itemtype="http://schema.org/Offer"{/if}>                                           	                                    	
                                            	<div class="precioIva">{if $node.class_identifier|eq('producto_mementix')}Desde {/if}{if $node.data_map.precio.content.has_discount}<s>{/if}{$node.data_map.precio.content.price|l10n(clean_currency)} € + IVA{if $node.data_map.precio.content.has_discount}</s>{/if}
{if $node.data_map.precio.content.has_discount}<br /><span class="price_offer" itemprop="price">{$node.data_map.precio.content.discount_price_ex_vat|l10n(clean_currency)}  € <span class="iva">+ IVA</span></span>{/if}<meta itemprop="priceCurrency" content="EUR" /></div>
                                            	
                                           </div>
                                          
                                           {if $node.class_identifier|eq('producto_mementix')}
                                              <a href={concat( 'basket/mementix')|ezurl} class="boton {if $node.class_identifier|eq('producto_mementix')|not}loQuiero{/if}"><img src={"btn_lo-quiero.png"|ezimage} alt="Lo quiero" /></a>
                                            {/if}
                                            {if $node.class_identifier|eq('producto')}
                                           <a href={concat( 'basket/ajaxadd/', $node.object.id, '/1')|ezurl} class="boton loQuiero"><img src={"btn_lo-quiero.png"|ezimage} alt="Lo quiero" /></a>
                                            {/if}
                                            {if $node.class_identifier|eq('producto_nautis4')}
                                                <a href={$node.url_alias|ezurl} class="boton"><img src={"btn_lo-quiero.png"|ezimage} alt="Lo quiero" /></a>
                                            {/if}
                                        
                                        </li>
