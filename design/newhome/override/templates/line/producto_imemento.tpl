								...
								{/if}
                                            ...
                                            <div class="action flt">                                           	                                    	
                                            	<div class="precioIva">{if $node.class_identifier|eq('producto_imemento')}Desde {/if}{if $node.data_map.precio.content.has_discount}<s>{/if}{$node.data_map.precio.content.price|l10n(clean_currency)} € + IVA{if $node.data_map.precio.content.has_discount}</s>{/if}
{if $node.data_map.precio.content.has_discount}<br /><span class="price_offer">{$node.data_map.precio.content.discount_price_ex_vat|l10n(clean_currency)}  € <span class="iva">+ IVA</span></span>{/if}</div>
                                           ...
                                           {if $node.class_identifier|eq('producto_imemento')}
                                              <a href={concat( 'basket/imemento')|ezurl} class="boton {if $node.class_identifier|eq('producto_imemento')|not}loQuiero{/if}"><img src={"btn_lo-quiero.png"|ezimage} alt="Lo quiero" /></a>
                                            {/if}
