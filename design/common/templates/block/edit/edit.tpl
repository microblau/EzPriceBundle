...
<div class="block-parameters float-break">
    ...
    {if $is_dynamic}
        ...
        {/if}
        {/foreach}
    {elseif or( $is_custom, $is_custom|not)}
        ...
        {foreach $custom_attributes as $custom_attrib}
            ...
            {if eq( $use_browse_mode[$custom_attrib], 'true' )}
                ...
                {def $nodoRelacionado=fetch('content','node',hash('node_id',$block.custom_attributes[$custom_attrib]|int))}
                <input class="textfield block-control" type="text" name="contenido" value="{$nodoRelacionado.name}" />
                {undef $nodoRelacionado}
            {else}
                <label>{$custom_titles[$custom_attrib]}:</label> 
        ...
        {/foreach}
        {if eq( ezini( $block.type, 'ManualAddingOfItems', 'block.ini' ), 'enabled' )}
